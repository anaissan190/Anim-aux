// supabase/functions/send-appointment-cancellation/index.ts
// Appelée par le trigger notify_email_sms_on_appointment_cancelled
// (migration 075) à chaque notification de type 'appointment_cancelled' —
// un praticien vient d'annuler un RDV confirmé (notification in-app déjà
// gérée par la migration 074). Complément email (Resend) + SMS (OVH), même
// mécanisme que send-appointment-confirmation / send-waitlist-email.
...
async function sendOvhSms(message: string, receiver: string) {
  ...
  // best-effort — l'annulation elle-même ne doit jamais en dépendre.
  try {
...
Deno.serve(async (req) => {
  ...
  // patient", indiscernable dans les logs.
  if (!resendKey) console.error('send-appointment-cancellation: RESEND_API_KEY manquant')
  let emailSent = false
  ...
  if (resendKey && patientEmail) {
    ...
    `
    // Même garde-fou timeout + try/catch que sendOvhSms ci-dessus, appliqué
    // ici après coup le 08/09/2026 (voir commentaire sur sendOvhSms).
    try {
